...
- (void)setKeyValues:(NSDictionary *)keyValues
{
    ...
    [self enumerateIvarsWithBlock:^(MJIvar *ivar, BOOL *stop) {
        ...
        {
            NSLog(@"为空 %@  %@",ivar.type.typeClass,value);
            if(ivar.type.typeClass == [NSString class])value = @""; else return;
//            return;
        }
        ...
        // 4.赋值
//        if([value isKindOfClass:ivar.type.typeClass])ivar.value = value;
        ivar.value = value;
    }];
...
- (NSDictionary *)keyValues
{
    ...
    [self enumerateIvarsWithBlock:^(MJIvar *ivar, BOOL *stop) {
        if (ivar.isSrcClassFromFoundation) return;
        if([ivar.propertyName isEqualToString:@"GL_requestData"] ||
           [ivar.propertyName isEqualToString:@"GL_dataClass"])return;
        // 1.取出属性值
        id value = ivar.value;
        if (!value)
        {
            Class objectClass = ivar.type.typeClass;
            value = [[objectClass alloc]init];
        }
